...
int main(int argc, char *argv[]) {
	...
	for (int i = length-2; i >= 0; --i) {
		dp_tab[i][0][LEFT] = (dp_tab[i+1][0+1][RIGHT] - pow(wind[i][0], 2));
		dp_tab[i][0][RIGHT] = dp_tab[i+1][0+1][RIGHT] + wind[i][0];
		for (int j = 1; j < width-1; ++j) {
			dp_tab[i][j][LEFT] = 
				max((int) dp_tab[i+1][j-1][LEFT] + wind[i][j], (int) (dp_tab[i+1][j+1][RIGHT] - pow(wind[i][j], 2)));
			dp_tab[i][j][RIGHT] = 
				max((int) dp_tab[i+1][j+1][RIGHT] + wind[i][j], (int) (dp_tab[i+1][j-1][LEFT] - pow(wind[i][j], 2)));
		}
		dp_tab[i][width-1][LEFT] = dp_tab[i+1][width-2][LEFT] + wind[i][width-1];
		dp_tab[i][width-1][RIGHT] = (dp_tab[i+1][width-2][LEFT] - pow(wind[i][width-1], 2));
	}
